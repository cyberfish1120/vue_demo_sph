<template>
  <div class="trade-container">
    ...
    <div class="sub clearFix">
      <a href="" class="subBtn" @click.prevent="submit">提交订单</a>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async submit() {
      ...
      const submitInfo = {
        ...
        orderDetailList: this.orderInfo.detailArrayList.map((detail) => {
          return {
            id: detail.id,
      ...
      try {
        await this.$store.dispatch("trade/submit", {
          tradeNo: this.orderInfo.tradeNo,
          data: submitInfo,
        });
        this.$router.push({ name: "pay", query:{orderId: this.$store.state.trade.orderId}});
      } catch (err) {
        alert(err);
      }
    },
